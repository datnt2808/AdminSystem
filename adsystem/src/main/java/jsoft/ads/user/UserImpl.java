...
public class UserImpl extends BasicImpl implements User {
	...
	private String createConditions(UserObject similar) {
		...
		if(similar!=null) {
			...
			int logined_id = similar.getUser_id();
			byte logined_permis = similar.getUser_permission();
			tmp = "((user_permission<="+logined_permis+") AND (logined_id="+logined_id+"))"; 
			tmp +=" OR (user_id="+logined_id+"))";
